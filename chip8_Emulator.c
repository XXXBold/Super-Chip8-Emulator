...
#define REG_PROGRAMCOUNTER_INCREMENT() REG_PC+=sizeof(word)
  ...
  #define MEM_SCREEN_BIT_CHECK(win,x,y) ((win).taScreenBuffer[(((win).data.exModeOn)?SUCHIP_SCREEN_INDEX(x,y):CHIP8_SCREEN_INDEX(x,y))]&(0x80>>((x)%8)))
  #define MEM_SCREEN_BYTE(win,x,y)       (win).taScreenBuffer[(((win).data.exModeOn)?SUCHIP_SCREEN_INDEX(x,y):CHIP8_SCREEN_INDEX(x,y))]
#else
               ...
               unsigned int callbackEvents)
{
  ...
  tagEmulator_g.tagWindow.data.scale=(winScaleFactor)?winScaleFactor:EMU_SCREEN_DEFAULT_SCALE;
  ...
  tagEmulator_g.updateSettings.threadEmu_Pause=0;
  tagEmulator_g.updateSettings.threadEmu_Quit=0;
  tagEmulator_g.updateSettings.threadEmu_Run=0;
  tagEmulator_g.updateSettings.threadUpdate_Pause=0;
  tagEmulator_g.updateSettings.threadUpdate_Quit=0;
  tagEmulator_g.updateSettings.threadUpdate_Run=0;
...
void chip8_SetPause(int pause)
{
  ...
  if(pause)
  {
    ...
    if(tagEmulator_g.tagThrdEmu.eEmuState==EMU_STATE_PAUSE)
      return;
    tagEmulator_g.updateSettings.threadEmu_Pause=1;
    while(tagEmulator_g.updateSettings.threadEmu_Pause);
  }
  else
  {
    ...
    tagEmulator_g.updateSettings.threadEmu_Run=1;
    /* Wait for thread to update setting */
    while((tagEmulator_g.updateSettings.screen_Show) || (tagEmulator_g.updateSettings.threadEmu_Run));
  }
}
...
void chip8_SetWindowScale(unsigned char windowScale)
{
  ...
  if(tagEmulator_g.tagWindow.data.scale==windowScale)
    return;

  tagEmulator_g.tagWindow.data.scale=windowScale;
...
void chip8_DumpScreen(void)
{
  int iIndex;
  int iIndexB;
  int iScreenWidth=(tagEmulator_g.tagWindow.data.exModeOn)?SUCHIP_SCREEN_WIDTH:CHIP8_SCREEN_WIDTH;
  int iScreenHeigth=(tagEmulator_g.tagWindow.data.exModeOn)?SUCHIP_SCREEN_HEIGHT:CHIP8_SCREEN_HEIGHT;
#ifdef ENABLE_SUPERCHIP_INSTRUCTIONS
...
int chip8_Process(TagEmulator *pEmulator)
{
  ...
  switch(tOPCode&0xF000)
  {
    case 0x0000: /* 0x0xxx */
      ...
      {
        tOPCode&=0x000F;
        iRc=((pEmulator->tagWindow.data.exModeOn)?SUCHIP_SCREEN_WIDTH:CHIP8_SCREEN_WIDTH)/8;
        ...
        TRACE_DBG_INFO_VARG("Scrolling down 0x%X lines, moving from data[0x0000]->[0x%.4X] (0x%.4X bytes), set data[0x0000]->[0x%.4X] to 0",
                            tOPCode,
                            iRc*tOPCode,
                            ((pEmulator->tagWindow.data.exModeOn)?SCREEN_SIZE_EXMODE:SCREEN_SIZE_NORMAL)-iRc*tOPCode,
                            iRc*tOPCode);
...
//        printf("Stackptr: 0x%X, startstack: 0x%X, Endstack: 0x%X\n",REG_PTR_STACK,OFF_ADDR_STACK_START,OFF_ADDR_STACK_END);
...
#ifdef ENABLE_SUPERCHIP_INSTRUCTIONS
        case SUCHIP_CMD_SCROLL_RIGHT:   // TODO: implement superchip instructions
          TRACE_SUCHIP_INSTR(TXT_INSTRUCTION_0x00FB,REG_PC,pEmulator->tCurrOPCode);
          tRegY=(pEmulator->tagWindow.data.exModeOn)?SUCHIP_SCREEN_HEIGHT:CHIP8_SCREEN_HEIGHT;
          tRegX=(pEmulator->tagWindow.data.exModeOn)?SUCHIP_SCREEN_WIDTH:CHIP8_SCREEN_WIDTH;
          EMU_LOCK_SCREEN(pEmulator);
        ...
        case SUCHIP_CMD_DIS_EX_SCREEN:
          ...
          pEmulator->updateSettings.screen_ExMode_Dis=1;   // TODO: wait for operation?
          while(pEmulator->updateSettings.screen_ExMode_Dis);
          iRc=RET_SUCHIP_OPCODE_OK;
          break;
        case SUCHIP_CMD_EN_EX_SCREEN:
          TRACE_SUCHIP_INSTR(TXT_INSTRUCTION_0x00FF,REG_PC,pEmulator->tCurrOPCode);
          pEmulator->updateSettings.screen_ExMode_En=1;   // TODO: wait for operation?
          while(pEmulator->updateSettings.screen_ExMode_En);
          iRc=RET_SUCHIP_OPCODE_OK;
          break;
#endif /* ENABLE_SUPERCHIP_INSTRUCTIONS */
        default:
    ...
    case 0x8000: /* Mathematical Operations */
      ...
      switch(tOPCode&0x000F)
      {
        ...
        case CHIP8_CMD_MATH_SHR: /* SHR */
          TRACE_CHIP8_INSTR(TXT_INSTRUCTION_0x8X06,REG_PC,pEmulator->tCurrOPCode,tRegX,tRegX,REG_VX(tRegX));
        ...
        case CHIP8_CMD_MATH_SHL: /* SHL */
          TRACE_CHIP8_INSTR(TXT_INSTRUCTION_0x8X0E,REG_PC,pEmulator->tCurrOPCode,tRegX,tRegX,REG_VX(tRegX));
    ...
    case 0xD000: /* 0xDxyn, Draw n Sprites @Pos Reg_Vx/REG_Vy */
      ...
      else
      {
        ...
        if(!pEmulator->tagWindow.data.exModeOn)
        {
    ...
    case 0xE000: /* Keypress stuff */
      ...
      switch((tOPCode&0x00FF))
      {
        case CHIP8_CMD_KEY_PRESSED: /* 0xEx9E, Skip next instruction if REG_Vx==pressed key */
          TRACE_CHIP8_INSTR(TXT_INSTRUCTION_0xEX9E,REG_PC,pEmulator->tCurrOPCode,tRegX,tRegX,REG_VX(tRegX));   // TODO: check if key works
        ...
        case CHIP8_CMD_KEY_RELEASED: /* 0xExA1, Skip next instruction if REG_Vx==released key */
          TRACE_CHIP8_INSTR(TXT_INSTRUCTION_0xEXA1,REG_PC,pEmulator->tCurrOPCode,tRegX,tRegX,REG_VX(tRegX));   // TODO: check if key works
          if(!(pEmulator->tagKeyboard.ulKeyStates&(1<<REG_VX(tRegX))))
          {
    ...
    case 0xF000: /* Misc stuff */
      ...
      switch((tOPCode&0x00FF))
      {
        ...
        case CHIP8_CMD_I_SPRITE_GET:  /* 0xFx29, Set I to the address from sprite of val REG_Vx */
          ...
          break;
#ifdef ENABLE_SUPERCHIP_INSTRUCTIONS   // TODO: implement superchip instructions
        ...
        case CHIP8_CMD_REGISTERS_STORE: /* 0xFx55, Store register V0-Vx in memory pointed by REG_I */
          TRACE_CHIP8_INSTR(TXT_INSTRUCTION_0xFX55,REG_PC,pEmulator->tCurrOPCode,tRegX,tRegX);
        ...
        case CHIP8_CMD_REGISTERS_READ: /* 0xFx65, Read Values for Registers V0-Vx from memory pointed by REG_I */
          TRACE_CHIP8_INSTR(TXT_INSTRUCTION_0xFX65,REG_PC,pEmulator->tCurrOPCode,tRegX,tRegX);
                               ...
                               byte tBytes) /* 0xDxyn */
{
  ...
  word tCurrVal;
#ifdef ENABLE_SUPERCHIP_INSTRUCTIONS
  const unsigned int uiScreenWidth=(pEmulator->tagWindow.data.exModeOn)?SUCHIP_SCREEN_WIDTH:CHIP8_SCREEN_WIDTH;
  const unsigned int uiScreenHeight=(pEmulator->tagWindow.data.exModeOn)?SUCHIP_SCREEN_HEIGHT:CHIP8_SCREEN_HEIGHT;
#else
  ...
  if((tPosX>uiScreenWidth-1) || (tPosY>uiScreenHeight-1))
  {
    TRACE_DBG_ERROR_VARG("Drawing Position(%u-%u/%u-%u) out of Screen(%u/%u), cancel drawing",
                         ...
                         uiScreenWidth-1,
                         uiScreenHeight-1);
    return(RET_ERR_SCREEN_DRAWPOS_INVALID);
  }
  REG_VF=0;
  EMU_LOCK_SCREEN(pEmulator);
