                 ...
                 const char *path)
{
  ...
  if(!(fp=fopen(path,"rb")))
  {
    ERR_PRINT_VARGS("fopen() failed (%d): %s",errno,strerror(errno));
    return(INI_ERR_IO);
  }
                                 ...
                                 TagData *ptagData)
{
  ...
  switch((iRc=Ini_ConvertValue_m(file->ptagCurrSection->ptagCurrKey,ptagData)))
  {
    case INI_ERR_NONE:
                                ...
                                const TagData *ptagData)
{
  ...
  switch(ptagData->eType&0xFF)
  {
    case eDataType_Int:
    case eDataType_Uint:
      switch(ptagData->eType&0xFF00)
      {
        case eDataRepresentation_Int_Binary:
          /* Skip leading zeros */
          ...
          break;
        case eDataRepresentation_Int_Octal:
          if((ptagData->eType & 0xFF) == eDataType_Int)
          ...
          ++iRc;
          break;
        case eDataRepresentation_Int_Decimal:
          if((ptagData->eType & 0xFF) == eDataType_Int)
          ...
          ++iRc;
          break;
        case eDataRepresentation_Int_Hexadecimal:
          if((ptagData->eType & 0xFF) == eDataType_Int)
          ...
          break;
        default:
          ERR_PRINT_VARGS("Ini_KeySetData_m(): Invalid eDataRepresentation (0x%X)",ptagData->eType);
  ...
  file->ptagCurrSection->ptagCurrKey->type=ptagData->eType;
  file->ptagCurrSection->ptagCurrKey->uiDataSize=iRc;
  return(INI_ERR_NONE);
}
                                  ...
                                  TagData *ptagData)
{
  ...
  {
    if(((ptagData->eType & 0xFF) == eDataType_Int)    ||
       ((ptagData->eType & 0xFF) == eDataType_Uint)   ||
       ((ptagData->eType & 0xFF) == eDataType_Double) ||
       ((ptagData->eType & 0xFF) == eDataType_Boolean))
    {
      return(INI_ERR_CONVERSION);
    }
  }
